...
def draw_page_one(pdf, data, force_alternate2=False):
    ...
    for label, key, label_x, rule_x in v_layout:
        ...
        _text(pdf, rule_x, 653.1 + shift, "_______________")
        _text(pdf, rule_x + 6, 651.6 + shift, v_speeds.get(key))
    ...
    def _row(cells, header=False):
        ...
        for index, cell in enumerate(cells):
            ...
            if header:
                ...
                _text_center(pdf, center, y + 14.2, cell)
...
def draw_final_page(pdf, data):
    ...
    for raw_line in str(flight_plan_text or "").split("\n"):
        ...
        if not raw_line:
            y += 11.25
            continue
        for wrapped in _wrap(raw_line, SIZE_FPL, 549.3 - 46.0) or [""]:
