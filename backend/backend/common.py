...
# --------------------------------------------------
# AIRPORT CITY NAME (DEP/DEST suffix)
# --------------------------------------------------
# Shared by every format that prints a DEP/DEST city next to the ICAO
# code (VTKCM, VTCSP/VTAHP, DEFAULT/DEFAULT 1, INDO PACIFIC, VTVIK,
# VTHYR) - one lookup table instead of six near-duplicate copies.

# ForeFlight's export never carries an airport's full name - anything not
# listed here falls back to the city printed against the nearest navaid
# in the navlog itself (see _navaid_city below).
AIRPORT_NAME_LOOKUP = {
    "VOHY": "BEGUMPET",
    "VOHB": "HUBLI",
    "VOGA": "GOA",
...
# The operator's own rule for when a DEP/DEST code is even eligible for a
# city name at all: 3-6 characters, starting with one of these two-letter
# prefixes. Anything else - including a code this lookup or the navaid
# fallback could otherwise name - prints as the bare code, no exceptions.
AIRPORT_CITY_PREFIXES = ("VT", "VO", "VM")


def _navaid_city(rows, from_end=False):
    """ForeFlight prints the nearest navaid's city against each waypoint
    ("AAE AHMEDABAD 113.1") - the first on the route is the departure
    city and the last the destination city, the only place an airport's
    full name appears anywhere in the export besides the lookup above."""
...
def airport_city_name(code, rows, from_end=False):
    """The city name for a DEP/DEST code, or "" when the operator's own
    prefix/length rule doesn't match, or when neither the lookup nor the
    navaid fallback can find one - callers print the bare code either
    way, this only ever adds to it, never replaces it."""
    code = value(code).upper()
    if not (3 <= len(code) <= 6 and code[:2] in AIRPORT_CITY_PREFIXES):
        return ""
    return AIRPORT_NAME_LOOKUP.get(code) or _navaid_city(rows, from_end=from_end)
